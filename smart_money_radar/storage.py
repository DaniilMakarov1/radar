...
class SQLiteStore:
    ...
    def record_dune_execution(
        ...
        with self.connect() as connection:
            connection.execute(
                ...
                """,
                (execution_id, source, sql_file, output_file, state, utc_now_iso()),
            )

    def finish_dune_execution(
        self,
        execution_id: str,
        state: str,
        ...
        with self.connect() as connection:
            connection.execute(
                ...
                WHERE execution_id = ?
                """,
                (state, utc_now_iso(), row_count, error, output_file, execution_id),
            )

    def dashboard_dune_executions(self, limit: int = 10) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                """
                SELECT
    ...
    def dashboard_summary(self) -> dict[str, Any]:
        with self.connect() as connection:
            ...
            dune_execution_count = connection.execute(
                "SELECT COUNT(*) FROM dune_executions"
            ).fetchone()[0]
        ...
        return {
            ...
            "dune_execution_count": dune_execution_count,
    ...
    def start_prediction_scan(self, config: dict[str, Any]) -> int:
        with self.connect() as connection:
            cursor = connection.execute(
                """
        ...
        with self.connect() as connection:
            connection.executemany(
                ...
                ON CONFLICT(venue, event_id) DO UPDATE SET
                    ...
                    augmented_neg_risk = excluded.augmented_neg_risk,
                ...
                [
                    (
                        ...
                        event.get("starts_at"),
                        event.get("closes_at"),
                        event.get("expected_resolution_at"),
                        event.get("resolution_source"),
                        event.get("resolution_rules"),
                        event.get("cancellation_rules"),
                        int(bool(event.get("mutually_exclusive"))),
                        int(bool(event.get("exhaustive"))),
                        int(bool(event.get("neg_risk"))),
                        int(bool(event.get("augmented_neg_risk"))),
                        int(bool(event.get("active"))),
                        event.get("source_url"),
                        event["observed_at"],
                        json.dumps(event.get("raw", {}), sort_keys=True),
                        now,
                    )
                    for event in events
                ],
            ...
            connection.executemany(
                ...
                [
                    (
                        ...
                        market.get("expected_resolution_at"),
                        market.get("resolution_source"),
                        market.get("resolution_rules"),
                        market.get("cancellation_rules"),
                        float(market.get("fee_rate") or 0),
                        float(market.get("maker_fee_rate") or 0),
                        int(bool(market.get("fees_enabled"))),
                        float(market.get("min_order_size") or 1),
                        float(market.get("tick_size") or 0.01),
                        float(market.get("volume") or 0),
                        float(market.get("volume_24h") or 0),
                        float(market.get("liquidity") or 0),
                        int(bool(market.get("accepting_orders"))),
                        int(bool(market.get("is_other_outcome"))),
                        market["observed_at"],
                        json.dumps(market.get("raw", {}), sort_keys=True),
                        now,
                    )
                    for market in markets
                ],
            )
        return {"events": len(events), "markets": len(markets)}

    def insert_prediction_orderbooks(
        self,
        scan_id: int,
        ...
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_orderbook_snapshots (
                    prediction_scan_id,
                    venue,
                    market_id,
                    observed_at,
                ...
                [
                    (
                        ...
                        book["market_id"],
                        book["observed_at"],
                        json.dumps(book.get("yes_bids", [])),
                        json.dumps(book.get("yes_asks", [])),
                        json.dumps(book.get("no_bids", [])),
                        json.dumps(book.get("no_asks", [])),
                        book.get("best_yes_bid"),
                        book.get("best_yes_ask"),
                        book.get("best_no_bid"),
                        book.get("best_no_ask"),
                        float(book.get("yes_bid_depth") or 0),
                        float(book.get("yes_ask_depth") or 0),
                        float(book.get("no_bid_depth") or 0),
                        float(book.get("no_ask_depth") or 0),
                        json.dumps(book.get("raw", {}), sort_keys=True),
                    )
                    for book in books
                ],
            )
    ...
    def insert_prediction_constraints(
        self,
        scan_id: int,
        rows: list[dict[str, Any]],
    ) -> int:
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_constraints (
                    prediction_scan_id,
                    venue,
                    antecedent_market_id,
                    consequent_market_id,
                    relation_type,
                    confidence_score,
                    source,
                    rationale_json,
                ...
                [
                    (
                        scan_id,
                        row["venue"],
                        row["antecedent_market_id"],
                        row["consequent_market_id"],
                        row["relation_type"],
                        row["confidence_score"],
                        row["source"],
    ...
    def insert_prediction_contract_matches(
        self,
        scan_id: int,
        rows: list[dict[str, Any]],
    ) -> int:
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_contract_matches (
                    prediction_scan_id,
                    venue_a,
                    market_id_a,
                    venue_b,
                ...
                [
                    (
                        ...
                        row["match_score"],
                        row["status"],
                        row.get("deadline_delta_hours"),
                        int(bool(row.get("cancellation_match"))),
                        json.dumps(row.get("rationale", [])),
            ...
            for route in routes:
                cursor = connection.execute(
                    ...
                    (
                        ...
                        route.get("event_id"),
                        route["status"],
                        route.get("confidence_score", 0),
                        route.get("semantic_match_score"),
                        route.get("guaranteed_payout_per_share", 0),
                        route.get("max_executable_size", 0),
                        route.get("optimal_size", 0),
                        route.get("gross_edge_per_share"),
                        route.get("net_edge_per_share"),
                        route.get("expected_gross_profit"),
                        route.get("expected_net_profit"),
                        route.get("capital_required"),
                        route.get("total_fees", 0),
                        route.get("slippage_cost", 0),
                        route.get("operations_buffer", 0),
                        route.get("capital_lock_days"),
                        route.get("annualized_return"),
                        route["observed_at"],
                        route.get("source_url"),
                        json.dumps(route.get("legs", [])),
                        json.dumps(route.get("rationale", [])),
                        json.dumps(route.get("risk_flags", [])),
                        json.dumps(route.get("evidence", {}), sort_keys=True),
                    ),
                )
                output.append(
    ...
    def previous_prediction_routes(
        self,
        before_scan_id: int,
        statuses: tuple[str, ...] = ("paper_candidate",),
    ) -> list[dict[str, Any]]:
        placeholders = ",".join("?" for _ in statuses)
        with self.connect() as connection:
            previous = connection.execute(
                """
                SELECT prediction_scan_id
            ...
            rows = connection.execute(
                ...
                """,
                (int(previous["prediction_scan_id"]), *statuses),
            ).fetchall()
        output = []
        for row in rows:
            item = dict(row)
    ...
    def insert_prediction_paper_executions(
        self,
        scan_id: int,
        rows: list[dict[str, Any]],
    ) -> int:
        ...
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_paper_executions (
                    prediction_route_id,
                    prediction_scan_id,
                    model_version,
                    status,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(prediction_route_id, model_version)
                DO UPDATE SET
                    prediction_scan_id = excluded.prediction_scan_id,
                    status = excluded.status,
                    ...
                    depth_haircut = excluded.depth_haircut,
                    queue_ahead_size = excluded.queue_ahead_size,
                    expected_net_profit = excluded.expected_net_profit,
                    simulated_net_profit = excluded.simulated_net_profit,
                    simulated_fees = excluded.simulated_fees,
                    simulated_slippage = excluded.simulated_slippage,
                ...
                [
                    (
                        ...
                        row["latency_ms"],
                        row["depth_haircut"],
                        row["queue_ahead_size"],
                        row.get("expected_net_profit"),
                        row.get("simulated_net_profit"),
                        row.get("simulated_fees", 0),
                        row.get("simulated_slippage", 0),
    ...
    def prune_prediction_history(self, keep_scans: int = 10_080) -> dict[str, int]:
        ...
        with self.connect() as connection:
            ...
            book_count = connection.execute(
                f"SELECT COUNT(*) FROM prediction_orderbook_snapshots WHERE prediction_scan_id IN ({placeholders})",
                old_ids,
            ).fetchone()[0]
            connection.execute(
                f"""
                DELETE FROM prediction_paper_executions
                WHERE prediction_scan_id IN ({placeholders})
                   OR prediction_route_id IN (
                       SELECT prediction_route_id
                       FROM prediction_routes
                       WHERE prediction_scan_id IN ({placeholders})
                   )
                """,
            ...
            for table in (
                ...
                connection.execute(
                    f"DELETE FROM {table} WHERE prediction_scan_id IN ({placeholders})",
                    old_ids,
                )
            connection.execute(
                f"DELETE FROM prediction_scans WHERE prediction_scan_id IN ({placeholders})",
                old_ids,
            )
        return {
    ...
    def upsert_prediction_wallet_positions(
        self,
        rows: list[dict[str, Any]],
    ) -> int:
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_wallet_positions (
                    venue,
                    wallet_address,
                    asset_id,
                    condition_id,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(venue, wallet_address, asset_id) DO UPDATE SET
                    condition_id = excluded.condition_id,
                ...
                [
                    (
                        ...
                        row.get("total_bought"),
                        row.get("realized_pnl"),
                        row.get("resolved_price"),
                        row.get("closed_at"),
                        json.dumps(row.get("raw", {}), sort_keys=True),
                        row["observed_at"],
                    )
    ...
    def upsert_prediction_wallet_scores(
        self,
        rows: list[dict[str, Any]],
    ) -> int:
        now = utc_now_iso()
        with self.connect() as connection:
            connection.executemany(
                """
                INSERT INTO prediction_wallet_scores (
                    venue,
                    wallet_address,
                    model_version,
                    user_name,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(venue, wallet_address, model_version) DO UPDATE SET
                    ...
                    shrunken_roi = excluded.shrunken_roi,
                    max_drawdown_usd = excluded.max_drawdown_usd,
                    profit_concentration = excluded.profit_concentration,
                ...
                [
                    (
                        ...
                        row.get("posterior_win_rate", 0),
                        row.get("total_bought_usd", 0),
                        row.get("total_realized_pnl_usd", 0),
                        row.get("realized_roi"),
                        row.get("shrunken_roi"),
                        row.get("max_drawdown_usd"),
                        row.get("profit_concentration"),
                        row.get("top_category"),
                        row.get("specialization_score"),
    ...
    def prediction_wallet_scores_are_fresh(
        self,
    ...
    def prediction_token_candidates(self) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                """
                SELECT
                    ...
                    t.name,
                    tc.chain_id,
                    tc.contract_address AS token_address
                FROM tokens t
                LEFT JOIN token_contracts tc
                    ON tc.token_contract_id = (
                        SELECT candidate.token_contract_id
                        FROM token_contracts candidate
                        WHERE candidate.token_id = t.token_id
                        ORDER BY
                            CASE candidate.chain_id
                                WHEN 'base' THEN 1
    ...
    def replace_prediction_event_token_links(
        ...
        with self.connect() as connection:
            ...
            connection.executemany(
                """
                INSERT INTO prediction_event_token_links (
                    venue,
                    event_id,
                    token_id,
                    chain_id,
    ...
    def prediction_dashboard(self, route_limit: int = 100) -> dict[str, Any]:
        with self.connect() as connection:
            ...
            if scan_id:
                ...
                route_counts = [
                    ...
                    for row in connection.execute(
                        """
                        SELECT route_type, status, COUNT(*) AS route_count
                        FROM prediction_routes
                        WHERE prediction_scan_id = ?
                ...
                paper_summary = dict(
                    connection.execute(
                        ...
                        SELECT
                            ...
                            SUM(CASE WHEN status LIKE 'filled_%' THEN 1 ELSE 0 END) AS filled_count,
                            SUM(CASE WHEN status LIKE 'partial_%' THEN 1 ELSE 0 END) AS partial_count,
                            SUM(CASE WHEN status IN ('filled_profitable', 'partial_profitable') THEN 1 ELSE 0 END) AS profitable_count,
                            AVG(fill_ratio) AS average_fill_ratio,
                            SUM(COALESCE(simulated_net_profit, 0)) AS simulated_net_profit,
                            SUM(COALESCE(simulated_fees, 0)) AS simulated_fees
                        FROM prediction_paper_executions
                        WHERE prediction_scan_id = ?
                        """,
                        (scan_id,),
                    ).fetchone()
                ...
                match_summary = dict(
                    connection.execute(
                        ...
                        SELECT
                            ...
                            SUM(cancellation_match) AS cancellation_matched_count,
                            MAX(match_score) AS best_match_score
                        FROM prediction_contract_matches
                        WHERE prediction_scan_id = ?
                        """,
                        (scan_id,),
                    ).fetchone()
                ...
                venue_rows = [
                    ...
                    for row in connection.execute(
                        ...
                        SELECT
                            book.venue,
                            COUNT(DISTINCT market.event_id) AS event_count,
                            COUNT(*) AS market_count,
                            COUNT(*) AS open_market_count,
                            SUM(market.volume_24h) AS volume_24h,
                            MAX(book.observed_at) AS observed_at
                        FROM prediction_orderbook_snapshots book
                        JOIN prediction_markets market
                          ON market.venue = book.venue
                         AND market.market_id = book.market_id
                        WHERE book.prediction_scan_id = ?
                        GROUP BY book.venue
                        ORDER BY book.venue
                        """,
                        (scan_id,),
            ...
            link_rows = connection.execute(
                ...
                SELECT
                    ...
                    event.source_url
                FROM prediction_event_token_links link
                JOIN prediction_events event
                  ON event.venue = link.venue
                 AND event.event_id = link.event_id
                ORDER BY link.confidence_score DESC, link.token_symbol
                LIMIT 30
                """
            ).fetchall()
