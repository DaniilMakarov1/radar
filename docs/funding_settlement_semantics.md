...
| Pacifica | mainnet | https://docs.pacifica.fi/trading-on-pacifica/funding-rates, https://docs.pacifica.fi/api-documentation/api/rest-api/markets/get-prices | `POSITION_AT_EVENT_FULL` | open position during hourly funding epoch | no | 1h | 1h | no | unknown until canary | public `funding_rate/history`; account `funding/history` | `RESEARCH_ONLY` | Official docs expose hourly funding, current/next rates, history, mark/oracle/OI/volume and public fee levels. Shadow/Paper remain blocked by `MAINNET_CANARY_REQUIRED` and `shadow_candidate_enabled=False`. |
| Nado | mainnet | https://docs.nado.xyz/core/funding-rates, https://docs.nado.xyz/developer-resources/api/archive-indexer/funding-rate-history | `PERIODIC_INDEX_STEP` | open position during hourly funding step | no | 1h | 24h displayed/predicted API rate | no | unknown until canary | Archive hourly funding rate history; interest/funding payments API | `RESEARCH_ONLY` | Archive latest funding is x18 24h predicted rate; adapter converts to hourly settlement rate with `Decimal` and stores raw x18/scale. Candidate/Paper blocked until inclusion/canary review. |
| Variational | mainnet | https://docs.variational.io/developer/api, https://docs.variational.io/developer/rfq | `UNKNOWN` | unknown | no | variable 1h-8h | interval-specific | yes | unknown | unknown | excluded | RFQ execution model. Research ingestion only; no settlement-capture route, hedge leg, synthetic CLOB orderbook, zero-fee assumption, PaperBot opening, or live execution. |
